...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    x_positions = [restaurant_location(x)[0] for x in cluster]
    y_positions = [restaurant_location(x)[1] for x in cluster]
    return [mean(x_positions), mean(y_positions)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xs_mean = mean(xs)
    ys_mean = mean(ys)
    sxx = sum([(x - xs_mean)**2 for x in xs])
    syy = sum([(x - ys_mean)**2 for x in ys])
    sxy = sum([(x[0] - xs_mean)*(x[1] - ys_mean) for x in zip(xs, ys)])
    b = sxy / sxx
    a = ys_mean - b * xs_mean
