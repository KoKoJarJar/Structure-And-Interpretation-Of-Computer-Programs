...
(define (longest-increasing-subsequence lst)
  (cond ((null? lst) lst)
        (else ((lambda (x y) (if (> (length x) (length y)) x y))
               (cons (car lst)
                     (longest-increasing-subsequence (filter (lambda (x) (> x (car lst)))
                                                             (cdr lst))))
...
(define (derive-exp exp var)
  (make-product (derive (base exp) var) (make-product (exponent exp) (make-exp (base exp) (- (exponent exp) 1))))
)
