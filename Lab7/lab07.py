...
def link_to_list(link):
    ...
    array = []
    current_list = link
    while current_list != Link.empty:
        array += [current_list.first]
...
def store_digits(n):
    ...
    def helper(n, current):
        if n == 0:
            return current
        return helper(n // 10, Link(n % 10, current))
...
def is_bst(t):
    ...
    False
    """
    def bst_property(t, f):
        if t.is_leaf():
            return t.label
        branch_labels = [x.label for x in t.branches]
        return f(f(branch_labels), t.label)
    ...
    if len(t.branches) == 1:
        child = t.branches[0]
        return t.label >= bst_property(child, max) or \
               t.label <= bst_property(child, min)
    left_child = t.branches[0]
    right_child = t.branches[1]
    return t.label >= bst_property(left_child, max) and \
           t.label <= bst_property(right_child, min)
...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        ...
        left_child = t.branches[0]
        right_child = t.branches[1]
        yield from in_order_traversal(left_child)
        yield t.label
        yield from in_order_traversal(right_child)
