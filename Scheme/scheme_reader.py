...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        function_string = ''
        if val == "'":
            function_string = 'quote'
        elif val == "`":
            function_string = 'quasiquote'
        else:
            function_string = 'unquote'
        return Pair(function_string, Pair(scheme_read(src), nil))
        # END PROBLEM 7
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            exp = scheme_read(src)
            if src.current() != ')':
                raise SyntaxError('invalid dot placement')
            src.remove_front()
            return exp
            # END PROBLEM 2
        else:
