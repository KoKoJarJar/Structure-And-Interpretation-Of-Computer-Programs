...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place and current_place is not hive:
            ...
            if potential_bee and distance >= self.min_range and distance <= self.max_range :
...
class WallAnt(Ant):
    """WallAnt has a lot of armor, otherwise, does nothing."""
...
# END Problem 13
    ...
    # BEGIN Problem 13
    implemented = True   # Change to True to view in the GUI
    has_queen_spawned = False
    # END Problem 13

    def __init__(self, armor=1):
        # BEGIN Problem 13
        if not QueenAnt.has_queen_spawned:
            QueenAnt.has_queen_spawned = True
    ...
    def action(self, colony):
        ...
        while current_place:
            ...
            if current_ant:
                if current_ant not in self.buffed_ants:
                    current_ant.damage *= 2
                    self.buffed_ants += [current_ant]
                if current_ant.is_container:
                    contained_ant = current_ant.contained_ant
                    if contained_ant and contained_ant not in self.buffed_ants:
                        contained_ant.damage *= 2
                        self.buffed_ants += [contained_ant]
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    def new_action(colony):
        if bee.place.entrance is not colony.hive:
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        nonlocal duration
        new_action_local = effect(old_action, bee)
        if duration > 0:
            duration -= 1
            return new_action_local(colony)
        else:
            return old_action(colony)
    if effect is make_scare and bee.scared_once:
        return
    if effect is make_scare and not bee.scared_once:
...
class LaserAnt(ThrowerAnt):
        ...
        while current_place is not hive:
            ...
            if current_ant:
                ...
                if current_ant.is_container and current_ant.contained_ant:
                    insects[current_ant.contained_ant] = distance
            for bee in current_place.bees:
                insects[bee] = distance
            current_place = current_place.entrance
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage = 2.0 - (distance * 0.2)
        damage -= self.insects_shot * 0.05
        damage = max(damage , 0)
        return damage
        # END Problem OPTIONAL
