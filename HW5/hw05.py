...
def replace_leaf(t, old, new):
    ...
    new_value = new if (label(t) == old and is_leaf(t)) else label(t)
...
def balanced(m):
    ...
    left_length, right_length = length(left_side), length(right_side)
    left_weight, right_weight = total_weight(end(left_side)), total_weight(end(right_side))
...
def totals_tree(m):
    ...
    new_branches = [left_tree, right_tree]
...
def multiple_references_explanation():
    return """The multiple reference problem occurs because mutiplice instances
    of the same reference will increase the error, accumulate and propagate the error
    throught the calculation"""
...
def quadratic(x, a, b, c):
    ...
    def helper(x):
        return a * x**2 + b * x + c
    extremum_x = (-b) / (2 * a)
    extremum_value = helper(extremum_x)
    check_extremum = False
    if extremum_value >= lower_bound(x) and extremum_value <= upper_bound(x):
        check_extremum = True
    left, right = helper(lower_bound(x)), helper(upper_bound(x))
    lower, upper = min(left, right), max(left, right)
    return interval(lower, upper) if not check_extremum \
        else interval(min(lower, extremum_value), max(upper, extremum_value))
