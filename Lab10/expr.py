...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operand_value = [x.eval(env) for x in self.operands]
        return operator_value.apply(operand_value)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        current_environment = self.parent.copy()
        index = 0
        for parameter in self.parameters:
            current_environment[parameter] = arguments[index]
            index += 1
        return self.body.eval(current_environment)
